...
import com.baidu.idl.face.platform.ui.CameraImageSource;
import com.baidu.idl.face.platform.ui.DetectRegionProcessor;
import com.baidu.idl.face.platform.ui.FaceDetectManager;
import com.baidu.idl.face.platform.ui.FaceFilter;
import com.baidu.idl.face.platform.ui.ImageFrame;
import com.baidu.idl.face.platform.ui.PreviewView;
import com.baidu.idl.face.platform.ui.camera.ICameraControl;
import com.baidu.idl.face.platform.ui.camera.PermissionCallback;
...
import com.example.administrator.myapplication.utils.ImageSaveUtil;
...
import com.example.administrator.myapplication.wigdet.BrightnessTools;
import com.example.administrator.myapplication.wigdet.FaceRoundView;
import com.example.administrator.myapplication.wigdet.WaveHelper;
import com.example.administrator.myapplication.wigdet.WaveView;
...
public class DetectLoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        faceDetectManager = new FaceDetectManager(this.getApplicationContext());
        initScreen();
        initView();
    ...
    private void initView() {
        ...
        //   previewView.getTextureView().setScaleX(-newhomepageone);
...
//                Color.parseColor("#3cf16d7a"));
     ...
     * 建议上传人脸到自己的服务器，在服务器端调用https://aip.baidubce.com/rest/newhomepagetwo.0/face/v2/identify，比对分数阀值（如：80分），
     * 认为登录通过
     * group_id	是	string	用户组id（由数字、字母、下划线组成），长度限制128B，如果需要查询多个用户组id，用逗号分隔
     * image	是	string	图像base64编码，每次仅支持单张图片，图片编码后大小不超过10M
     * ext_fields	否	string	特殊返回信息，多个用逗号分隔，取值固定: 目前支持 faceliveness(downdetection)，活体检测参考分数0.834963
     * 返回登录认证的参数给客户端
     *
     * @param model
     */
